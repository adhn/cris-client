            ...
            <v-flex xs12>
                <v-card>
                    <v-card-title>
                        ...
                        <project-contacts-attach-dialog />
                        <project-contacts-new-dialog />
                    </v-card-title>
                    <v-card-text>
                        <v-data-table
                        ...
                        >
                            ...
                            <template slot="items" slot-scope="props">
                                ...
                                <td><project-role-cell-render :project_role_id="getProjectRoleId(props.item)" /></td>
...
<script>
    ...
    import ProjectContactsAttachDialog from '~/components/Project/ProjectContactsAttachDialog.vue'
    import ProjectContactsNewDialog from '~/components/Project/ProjectContactsNewDialog.vue'

    export default {
        props: ['projectId'],
        components: {
            PhonesCellRender,EmailCellRender,ProjectRoleCellRender,ProjectContactsAttachDialog,ProjectContactsNewDialog
        },
        data: () => ({
            mustSort:true,
            headers: [
        ...
        methods: {
            getProjectRoleId(contact) {
                if(contact && contact.projects ) {
                    let project = contact.projects.find(function(element) {
                        // console.info(this)
                        return element.id === this.$store.getters['project/id']
