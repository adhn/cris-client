...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapGetters('audit_list',['auditsLoading','audits','total']),

            pagination: {
                get() { return this.$store.getters['audit_list/pagination']},
                set(val) {this.$store.commit('audit_list/pagination',val)}
            },
            params() {
                let p = this.$store.getters['audit_list/pagination'];
                return {
                    page: p.page,
        ...
        methods: {
            resetFilters() {
                this.$store.commit('audit_list/resetFilters')
            }
        }
    }
</script>